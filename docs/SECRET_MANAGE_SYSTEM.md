...
CI 조회 API 주소는 공통 Action이 사용하는 `https://secrets.homelab.robinjoon.xyz`로 제안하며 아직 배포된 서비스는 아니다. 하네스는 HTTPS 경로와 호출 job에서의 도달 가능성을 준비한다. 운영자 UI·관리 API도 HTTPS로 접근하며, SMS가 정한 운영자 인증에 필요한 배포 설정만 하네스에 전달한다. 네트워크 접근 수단은 호출자 인증과 별도로 구성한다.
...
아래는 구현 후 더미 값으로 확인할 계약 시험이며 아직 실행하지 않았다. 내부 테이블·라이브러리·관리 도구의 구조를 검사하지 않고 호출자가 관찰하는 결과를 확인한다.
